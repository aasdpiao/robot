...
function table.contain(body,sub)
...
function get_week_interval(timestamp)
    local temp = os.date("*t", timestamp)
    local daytimes = {0,6,5,4,3,2,1}
    local remain_day = daytimes[temp.wday] 
    return os.time{year=temp.year, month=temp.month, day=temp.day, hour=23} + 60 * 60 + remain_day * 24 * 60 * 60
end
